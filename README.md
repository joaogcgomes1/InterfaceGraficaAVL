...
* **1. Modelo da Aeronave (Carregar Geometria .AVL):** Carrega as dimensões, superfícies e perfis da aeronave.
* **1. Modelo da Aeronave (Carregar Inércia .MASS):** Lê o arquivo contendo massas, centro de gravidade e tensores de inércia.
* **2. Configuração do Caso de Voo (Carregar arquivo .RUN):** Importa configurações previamente elaboradas no formato tradicional do AVL.
* **2. Configuração do Caso de Voo (Configuração Manual):** Permite definir diretamente na interface as condições cinemáticas (fixar Sustentação ou Ângulo de Ataque $\alpha$, ângulo de derrapagem $\beta$, Mach, taxas de rotação p, q, r) e escolher entre definir o ângulo (deflexão) das superfícies de controle ou transformá-las em variáveis de trimagem (ex: ajustar o profundor automaticamente para zerar o momento de arfagem $C_m$). O programa irá gerar um arquivo `.run` automático baseado nestas escolhas.
...
> **Nota:** As superfícies de comando são mostradas na geometria por meio de seções de cor mais escura nas superfícies geradas.
...
Esta janela apresenta os coeficientes aerodinâmicos para a condição de voo simulada. As principais informações disponíveis são:

* **Coeficientes aerodinâmicos:** Valores finais de $C_L$, $C_D$, $C_Y$, $C_l$, $C_m$ e $C_n$ para verificar a condição de equilíbrio.
* **Ponto Neutro manche fixo ($X_{np}$):** Representa a posição do centro de gravidade que torna a estabilidade estática longitudinal neutra.
* **Estabilidade Espiral:** Calculada a partir de $C_{l_b} \cdot C_{n_r} / C_{l_r} \cdot C_{n_b}$. O software indica visualmente com "Estável" (verde) ou "Instável" (vermelho) dependendo o valor obtido.
* **Deflexão das superfícies de comando:** Deflexões, em graus, que os comandos precisaram atingir (caso não tenham sido travados manualmente) para manter o voo trimado.

### Derivadas Aerodinâmicas
O AVL extrai um amplo conjunto de derivadas estabilidade. Nesta sub-aba, a interface separa as derivadas agrupando-as por coeficientes aerodinâmicos. 

Todas as derivadas são mostradas em **rad⁻¹**. 
* **Observação:** O AVL originalmente exporta as derivadas em relação à deflexão das superfícies de comando na unidade **°⁻¹** (1/grau). As derivadas mostradas na interface correspondem ao valor corrigido para **rad⁻¹**.
